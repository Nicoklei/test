...
class MainWindow(QMainWindow):
	...
	def __init__(self):
		...
		global path
		global path_folder
		path_folder = '/home/loaded_cell_qc/sciebo - Klein, Nico (s6nnklei@uni-bonn.de)@uni-bonn.sciebo.de/CERN_Doku/P1/P1_CoolingBlockFoto/'
		global up_down
		up_down = 0
	...
	def start_edit(self):
		...
		#print(serial_number)
		serial_number = int(path[15:19])
		print(serial_number, path, path[15:19])
		#print(serial_number)
		...
		#if os.path.exists("Cooling_Block_1.%i-1.%i"%(serial_number,serial_number+89)) ==True:
		...
		if up_down == 2:
			...
			for horizontal in range(9):
				...
				for vertical in range(10):
					...
					Cell1.save("Cooling_Block_pictures/1.%i.png"%(serial_number+vertical+horizontal*9+counter))
				counter += 1
		elif up_down == 0:
			counter = 0
			for horizontal in range(9):
				...
				counter += 1
		path = 'CoolingBlock_1-%i-%i'%(serial_number+90, serial_number+179)
