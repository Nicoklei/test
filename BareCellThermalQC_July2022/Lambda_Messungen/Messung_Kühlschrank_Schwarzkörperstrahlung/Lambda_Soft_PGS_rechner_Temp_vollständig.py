...
for i in temps:
   ...
   if i >= -49 and i <= -30:
      surout = 309.2#93.7#75.2#144.9#75.2#144.9
   elif i >= -29 and i <= -10:
      surout = 278.7#90.5#86.7#133.7#86.7#133.7
   elif i >= -9 and i <= 9:
      surout = 145.4#100#98.6#125.4#98.6#125.4
   elif i >= 10 and i <= 30:
      surout = 189#107.6#106.3#111.5#106.3
   ...
   datafile1 = np.loadtxt('%sC_4Vpro_Messung_Kühlschrank/Gemittelte_Werte_CERN09_%s.txt'%(i,i), delimiter='\t', unpack=True)
   A1,dA1,B1,dB1 = datafile1[2],datafile1[3],datafile1[4],datafile1[5]


   data1 = np.loadtxt('%sC_4Vpro_Messung_Kühlschrank/Messung0_CERN09_%s.txt'%(i,i), delimiter='\t', unpack=True)
   E1,dE1,F1,dF1,G1,dG1,H1,dH1=data1[0],data1[1],data1[2],data1[3],data1[4],data1[5],data1[6],data1[7]
   data2 = np.loadtxt('%sC_4Vpro_Messung_Kühlschrank/Messung1_CERN09_%s.txt'%(i,i), delimiter='\t', unpack=True)
   E2,dE2,F2,dF2,G2,dG2,H2,dH2=data2[0],data2[1],data2[2],data2[3],data2[4],data2[5],data2[6],data2[7]
   data3 = np.loadtxt('%sC_4Vpro_Messung_Kühlschrank/Messung2_CERN09_%s.txt'%(i,i), delimiter='\t', unpack=True)
   E3,dE3,F3,dF3,G3,dG3,H3,dH3=data3[0],data3[1],data3[2],data3[3],data3[4],data3[5],data3[6],data3[7]
   data4 = np.loadtxt('%sC_4Vpro_Messung_Kühlschrank/Messung3_CERN09_%s.txt'%(i,i), delimiter='\t', unpack=True)
   E4,dE4,F4,dF4,G4,dG4,H4,dH4=data4[0],data4[1],data4[2],data4[3],data4[4],data4[5],data4[6],data4[7]
   data5 = np.loadtxt('%sC_4Vpro_Messung_Kühlschrank/Messung4_CERN09_%s.txt'%(i,i), delimiter='\t', unpack=True)
   E5,dE5,F5,dF5,G5,dG5,H5,dH5=data5[0],data5[1],data5[2],data5[3],data5[4],data5[5],data5[6],data5[7]
   data6 = np.loadtxt('%sC_4Vpro_Messung_Kühlschrank/Messung5_CERN09_%s.txt'%(i,i), delimiter='\t', unpack=True)
...
#E6,dE6,F6,dF6,G6,dG6,H6,dH6 = np.append(E6,data6[0]),np.append(dE6,data6[1]),np.append(F6,data6[2]),np.append(dF6,data6[3]),np.append(G6,data6[4]),np.append(dG6,data6[5]),np.append(H6,data6[6]),np.append(dH6,data6[7])
   ...
   J1=sigma*(A*(273.15+I1)**4-surout/10000*2*(273.15+Tout)**4)
   J2=sigma*(A*(273.15+I2)**4-surout/10000*2*(273.15+Tout)**4)
   J3=sigma*(A*(273.15+I3)**4-surout/10000*2*(273.15+Tout)**4)
   J4=sigma*(A*(273.15+I4)**4-surout/10000*2*(273.15+Tout)**4)
   J5=sigma*(A*(273.15+I5)**4-surout/10000*2*(273.15+Tout)**4)
   J6=sigma*(A*(273.15+I6)**4-surout/10000*2*(273.15+Tout)**4)
...
#print(L1[0]/10000,L1[2]/10000,L1[4]/10000,L1[6]/10000,O1/10000,sigma*(273.15+24)**4/10000,sigma*93.2*(273.15+24)**4/10000)
   ...
   K1=(11.727-((L1[0]+L1[2]+L1[4]+L1[6]+O1)-sigma*surout*2*(273.15+Tout)**4)/10000)*0.05/(0.01**2*np.pi*(np.sum(E1)/len(E1)-np.sum(F1)/len(F1)))
   K2=(11.727-((L2[0]+L2[2]+L2[4]+L2[6]+O2)-sigma*surout*2*(273.15+Tout)**4)/10000)*0.05/(0.01**2*np.pi*(np.sum(E2)/len(E2)-np.sum(F2)/len(F2)))
   K3=(11.727-((L3[0]+L3[2]+L3[4]+L3[6]+O3)-sigma*surout*2*(273.15+Tout)**4)/10000)*0.05/(0.01**2*np.pi*(np.sum(E3)/len(E3)-np.sum(F3)/len(F3)))
   K4=(11.727-((L4[0]+L4[2]+L4[4]+L4[6]+O4)-sigma*surout*2*(273.15+Tout)**4)/10000)*0.05/(0.01**2*np.pi*(np.sum(E4)/len(E4)-np.sum(F4)/len(F4)))
   K5=(11.727-((L5[0]+L5[2]+L5[4]+L5[6]+O5)-sigma*surout*2*(273.15+Tout)**4)/10000)*0.05/(0.01**2*np.pi*(np.sum(E5)/len(E5)-np.sum(F5)/len(F5)))
   K6=(11.727-((L6[0]+L6[2]+L6[4]+L6[6]+O6)-sigma*surout*2*(273.15+Tout)**4)/10000)*0.05/(0.01**2*np.pi*(np.sum(E6)/len(E6)-np.sum(F6)/len(F6)))

   dK1=0.05/(0.01**2*np.pi*(np.sum(E1)/len(E1)-np.sum(F1)/len(F1)))*((L1[0]+L1[2]+L1[4]+L1[6]+O1)-sigma*surout*2*(273.15+Tout)**4)/10000*0.02
   dK2=0.05/(0.01**2*np.pi*(np.sum(E2)/len(E2)-np.sum(F2)/len(F2)))*((L2[0]+L2[2]+L2[4]+L2[6]+O2)-sigma*surout*2*(273.15+Tout)**4)/10000*0.02
   dK3=0.05/(0.01**2*np.pi*(np.sum(E3)/len(E3)-np.sum(F3)/len(F3)))*((L3[0]+L3[2]+L3[4]+L3[6]+O3)-sigma*surout*2*(273.15+Tout)**4)/10000*0.02
   dK4=0.05/(0.01**2*np.pi*(np.sum(E4)/len(E4)-np.sum(F4)/len(F4)))*((L4[0]+L4[2]+L4[4]+L4[6]+O4)-sigma*surout*2*(273.15+Tout)**4)/10000*0.02
   dK5=0.05/(0.01**2*np.pi*(np.sum(E5)/len(E5)-np.sum(F5)/len(F5)))*((L5[0]+L5[2]+L5[4]+L5[6]+O5)-sigma*surout*2*(273.15+Tout)**4)/10000*0.02
   dK6=0.05/(0.01**2*np.pi*(np.sum(E6)/len(E6)-np.sum(F6)/len(F6)))*((L6[0]+L6[2]+L6[4]+L6[6]+O6)-sigma*surout*2*(273.15+Tout)**4)/10000*0.02
...
#curve-fit-program
   ...
   popt1,pcov1 = curve_fit(func,II,A1)
   errors1 = np.sqrt(np.diag(pcov1))
   popt2,pcov2 = curve_fit(func,II,KK)
   errors2 = np.sqrt(np.diag(pcov2))
   #popt3,pcov3 = curve_fit(func,I3,A1)
   #errors3 = np.sqrt(np.diag(pcov3))
   #popt4,pcov4 = curve_fit(func,I4,A1)
   #errors4 = np.sqrt(np.diag(pcov4))
   #popt5,pcov5 = curve_fit(func,I5,A1)
   #errors5 = np.sqrt(np.diag(pcov5))
   #popt6,pcov6 = curve_fit(func,I6,A1)
...
#creat fit
   xlin1 = np.linspace(min(II),max(II),1000)
   ylin1 = func(xlin1,*popt1)
   xlin2 = np.linspace(min(II),max(II),1000)
   ylin2 = func(xlin2,*popt2)
   #xlin3 = np.linspace(min(I3),max(I3),1000)
   #ylin3 = func(xlin3,*popt3)
   #xlin4 = np.linspace(min(I4),max(I4),1000)
   #ylin4 = func(xlin4,*popt4)
   #xlin5 = np.linspace(min(I5),max(I5),1000)
   #ylin5 = func(xlin5,*popt5)
   #xlin6 = np.linspace(min(I6),max(I6),1000)
   #ylin6 = func(xlin6,*popt6)
...
#ax.set_facecolor('gainsboro')
   ...
   plt.errorbar(I1,A1[0],xerr=dI1,yerr=dA1[0], color='%s'%(color),fmt='.',label='T=%s'%(i))#'m = %s +/- %s \nn = %s +/- %s \nd = %s +/- %s'%(str(popt[0].round(5)),str(errors[0].round(5)),str(popt[1].round(5)),str(errors[1].round(5)),str(popt[2].round(5)),str(errors[2].round(5))))
   plt.errorbar(I2,A1[1],xerr=dI2,yerr=dA1[1], color='%s'%(color),fmt='.')#,label='Datenpunkte aus zweiter Messung')#'m = %s +/- %s \nn = %s +/- %s \nd = %s +/- %s'%(str(popt[0].round(5)),str(errors[0].round(5)),str(popt[1].round(5)),str(errors[1].round(5)),str(popt[2].round(5)),str(errors[2].round(5))))
   plt.errorbar(I3,A1[2],xerr=dI3,yerr=dA1[2], color='%s'%(color),fmt='.')#,label='Datenpunkte aus erster Messung')#'m = %s +/- %s \nn = %s +/- %s \nd = %s +/- %s'%(str(popt[0].round(5)),str(errors[0].round(5)),str(popt[1].round(5)),str(errors[1].round(5)),str(popt[2].round(5)),str(errors[2].round(5))))
   plt.errorbar(I4,A1[3],xerr=dI4,yerr=dA1[3], color='%s'%(color),fmt='.')#,label='Datenpunkte aus erster Messung')#'m = %s +/- %s \nn = %s +/- %s \nd = %s +/- %s'%(str(popt[0].round(5)),str(errors[0].round(5)),str(popt[1].round(5)),str(errors[1].round(5)),str(popt[2].round(5)),str(errors[2].round(5))))
   plt.errorbar(I5,A1[4],xerr=dI5,yerr=dA1[4], color='%s'%(color),fmt='.')#'m = %s +/- %s \nn = %s +/- %s \nd = %s +/- %s'%(str(popt[0].round(5)),str(errors[0].round(5)),str(popt[1].round(5)),str(errors[1].round(5)),str(popt[2].round(5)),str(errors[2].round(5))))
   plt.errorbar(I6,A1[5],xerr=dI6,yerr=dA1[5], color='%s'%(color),fmt='.')#,label='Datenpunkte aus erster Messung')#'m = %s +/- %s \nn = %s +/- %s \nd = %s +/- %s'%(str(popt[0].round(5)),str(errors[0].round(5)),str(popt[1].round(5)),str(errors[1].round(5)),str(popt[2].round(5)),str(errors[2].round(5))))

   plt.errorbar(I1,K1,xerr=dI1,yerr=dK1, color='%s'%(color),fmt='.',label='T=%s'%(i))#'m = %s +/- %s \nn = %s +/- %s \nd = %s +/- %s'%(str(popt[0].round(5)),str(errors[0].round(5)),str(popt[1].round(5)),str(errors[1].round(5)),str(popt[2].round(5)),str(errors[2].round(5))))
   plt.errorbar(I2,K2,xerr=dI2,yerr=dK2, color='%s'%(color),fmt='.')#,label='Datenpunkte aus zweiter Messung')#'m = %s +/- %s \nn = %s +/- %s \nd = %s +/- %s'%(str(popt[0].round(5)),str(errors[0].round(5)),str(popt[1].round(5)),str(errors[1].round(5)),str(popt[2].round(5)),str(errors[2].round(5))))
   plt.errorbar(I3,K3,xerr=dI3,yerr=dK3, color='%s'%(color),fmt='.')#,label='Datenpunkte aus erster Messung')#'m = %s +/- %s \nn = %s +/- %s \nd = %s +/- %s'%(str(popt[0].round(5)),str(errors[0].round(5)),str(popt[1].round(5)),str(errors[1].round(5)),str(popt[2].round(5)),str(errors[2].round(5))))
   plt.errorbar(I4,K4,xerr=dI4,yerr=dK4, color='%s'%(color),fmt='.')#,label='Datenpunkte aus erster Messung')#'m = %s +/- %s \nn = %s +/- %s \nd = %s +/- %s'%(str(popt[0].round(5)),str(errors[0].round(5)),str(popt[1].round(5)),str(errors[1].round(5)),str(popt[2].round(5)),str(errors[2].round(5))))
   plt.errorbar(I5,K5,xerr=dI5,yerr=dK5, color='%s'%(color),fmt='.')#'m = %s +/- %s \nn = %s +/- %s \nd = %s +/- %s'%(str(popt[0].round(5)),str(errors[0].round(5)),str(popt[1].round(5)),str(errors[1].round(5)),str(popt[2].round(5)),str(errors[2].round(5))))
   plt.errorbar(I6,K6,xerr=dI6,yerr=dK6, color='%s'%(color),fmt='.')#,label='Datenpunkte aus erster Messung')#'m = %s +/- %s \nn = %s +/- %s \nd = %s +/- %s'%(str(popt[0].round(5)),str(errors[0].round(5)),str(popt[1].round(5)),str(errors[1].round(5)),str(popt[2].round(5)),str(errors[2].round(5))))

   plt.plot(xlin1,ylin1,color='%s'%(color),lw=1.2)
   plt.plot(xlin2,ylin2,color='%s'%(color),lw=1.2)
   #plt.plot(xlin3,ylin3,color='%s'%(color),lw=1.2)
   #plt.plot(xlin4,ylin4,color='%s'%(color),lw=1.2)
   #plt.plot(xlin5,ylin5,color='%s'%(color),lw=1.2)
   #plt.plot(xlin6,ylin6,color='%s'%(color),lw=1.2)
