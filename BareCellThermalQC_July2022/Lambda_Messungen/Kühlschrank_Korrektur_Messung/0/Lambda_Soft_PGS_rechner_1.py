...
#define Function
def func(x,m,n):
   return 5.670*10**(-8)*(x**4*0.0115-n*(273.15)**4)+m**2
...
popt,pcov,chi = fit_basic(func,D+273.15,E,p0=[0,0.01],y_err=dE)
...
plt.errorbar(D+273.15,E,xerr=dD,yerr=dE, color='royalblue',fmt='.',label='m = (%s +/- %s)m² \nn = (%s +/- %s)m² \nchi²= %s'%(str((popt[0].round(1)).round(5)),str((pcov[0].round(1)).round(5)),str((popt[1].round(5)).round(5)),str((pcov[1].round(5)).round(5)),str(chi.round(2))))
...
plt.plot(xlin,ylin,color='firebrick', label=r'Fit-Kurve: $\sigma (x^4 \cdot 0.0115-n)$',lw=1.2)
...
plt.tight_layout()
plt.savefig("Fläche_aus_Leistung_0_2f")
