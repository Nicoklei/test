...
popt,pcov,chi = fit_basic(func,D+273.15,E,p0=[0.01,1000000],y_err=dE)
errors = np.sqrt(np.diag(pcov))
#creat fit
...
plt.errorbar(D+273.15,E,xerr=dD,yerr=dE, color='royalblue',fmt='.',label='m = (%s +/- %s)m² \nn = (%s +/- %s)m² \nchi²= %s'%(str(popt[0].round(5)),str(pcov[0].round(5)),str(((popt[1].round(10))/(273.15-40)**4).round(5)),str(((pcov[1].round(10))/(273.15-40)**4).round(5)),str(chi.round(2))))
...
plt.tight_layout()
plt.savefig("Fläche_aus_Leistung")
