...
datafile = np.loadtxt('Messung0_Black_body_-20.txt', delimiter='\t', unpack=True)
A,dA,B,dB,C=datafile[a],datafile[da],datafile[b],datafile[db],datafile[c]
datafile1 = np.loadtxt('Messung1_Black_body_-20.txt', delimiter='\t', unpack=True)
A1,dA1,B1,dB1,C1=datafile1[a],datafile1[da],datafile1[b],datafile1[db],datafile1[c]
datafile2 = np.loadtxt('Messung2_Black_body_-20.txt', delimiter='\t', unpack=True)
A2,dA2,B2,dB2,C2=datafile2[a],datafile2[da],datafile2[b],datafile2[db],datafile2[c]
datafile3 = np.loadtxt('Messung3_Black_body_-20.txt', delimiter='\t', unpack=True)
A3,dA3,B3,dB3,C3=datafile3[a],datafile3[da],datafile3[b],datafile3[db],datafile3[c]
datafile4 = np.loadtxt('Messung4_Black_body_-20.txt', delimiter='\t', unpack=True)
A4,dA4,B4,dB4,C4=datafile4[a],datafile4[da],datafile4[b],datafile4[db],datafile4[c]
datafile5 = np.loadtxt('Messung5_Black_body_-20.txt', delimiter='\t', unpack=True)
A5,dA5,B5,dB5,C5=datafile5[a],datafile5[da],datafile5[b],datafile5[db],datafile5[c]
datafile6 = np.loadtxt('Messung6_Black_body_-20.txt', delimiter='\t', unpack=True)
...
D = np.array([(A+B)/2,(A1+B1)/2,(A2+B2)/2,(A3+B3)/2,(A4+B4)/2,(A5+B5)/2,(A6+B6)/2])
dD = [(dA+dB)/2,(dA1+dB1)/2,(dA2+dB2)/2,(dA3+dB3)/2,(dA4+dB4)/2,(dA5+dB5)/2,(dA6+dB6)/2]
E = [C,C1,C2,C3,C4,C5,C6]
dE = [0.044,0.02,0.025,0.0313,0.077,0.0598,0.15]
...
plt.errorbar(D+273.15,E,xerr=dD,yerr=dE, color='royalblue',fmt='.',label='m = (%s +/- %s) \nn = (%s +/- %s)m² \nchi²= %s'%(str((popt[0].round(5)).round(5)),str((pcov[0].round(5)).round(5)),str((popt[1].round(5)).round(5)),str((pcov[1].round(5)).round(5)),str(chi.round(2))))
...
plt.plot(xlin,ylin,color='firebrick', label=r'Fit-Kurve: $\sigma (x^4 \cdot 0.0115-n \cdot (253.15)^4)$',lw=1.2)
...
plt.tight_layout()
plt.savefig("Fläche_aus_Leistung_-20_2f")
