...
D = np.array([(A+B)/2,(A1+B1)/2,(A2+B2)/2,(A3+B3)/2+2,(A4+B4)/2+2,(A5+B5)/2,(A6+B6)/2])
...
popt,pcov,chi = fit_basic(func,D+273.15,E,p0=[10,10],y_err=dE)
...
plt.errorbar(D+273.15,E,xerr=dD,yerr=dE, color='royalblue',fmt='.',label='m = (%s +/- %s)m² \nn = (%s +/- %s)m² \nchi²= %s'%(str(popt[0].round(5)),str(pcov[0].round(5)),str((popt[1].round(1)/(273.15-20)**4).round(5)),str((pcov[1].round(1)/(273.15-20)**4).round(5)),str(chi.round(2))))
...
plt.tight_layout()
plt.savefig("Fläche_aus_Leistung_1")
