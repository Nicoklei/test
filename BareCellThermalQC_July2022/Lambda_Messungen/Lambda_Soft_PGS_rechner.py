...
path_d = 'Soft-PGS_06_09/Gemittelte_Werte_Soft-PGS.txt'
...
path_d1 = 'Soft-PGS_23_08/Messung_Lambda_PGS_23_08_test.txt'
...
#curve-fit-program
popt,pcov = curve_fit(func1,A1,B1)
errors = np.sqrt(np.diag(pcov))
#creat fit
xlin = np.linspace(min(A1),max(A1),1000)
ylin = func1(xlin,*popt)

popt1,pcov1 = curve_fit(func,A4,B4,p0=[0.0001,0,7])
errors1 = np.sqrt(np.diag(pcov1))
#creat fit
xlin1 = np.linspace(min(A4),max(A4),1000)
ylin1 = func(xlin1,*popt1)

popt2,pcov2 = curve_fit(func,A5,A6,p0=[0.0001,0,7])
errors2 = np.sqrt(np.diag(pcov2))
#creat fit
xlin2 = np.linspace(min(A5),max(A5),1000)
...
plt.plot(xlin2,ylin2,color='navy', label=r'Fit-Kurve: $n\cdot e^{-m\cdot x}+d$',lw=1.2)
...
plt.tight_layout()
plt.savefig("Soft-PGS_alle_Werte_ein_Fit_mit_erstem_Wert")
