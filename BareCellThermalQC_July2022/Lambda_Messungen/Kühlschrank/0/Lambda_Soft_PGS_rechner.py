...
datafile = np.loadtxt('Messung0_Black_body_0.txt', delimiter='\t', unpack=True)
A,dA,B,dB,C,Temp=datafile[a],datafile[da],datafile[b],datafile[db],datafile[c],datafile[4]
datafile1 = np.loadtxt('Messung1_Black_body_0.txt', delimiter='\t', unpack=True)
A1,dA1,B1,dB1,C1,Temp1=datafile1[a],datafile1[da],datafile1[b],datafile1[db],datafile1[c],datafile1[4]
datafile2 = np.loadtxt('Messung2_Black_body_0.txt', delimiter='\t', unpack=True)
A2,dA2,B2,dB2,C2,Temp2=datafile2[a],datafile2[da],datafile2[b],datafile2[db],datafile2[c],datafile2[4]
datafile3 = np.loadtxt('Messung3_Black_body_0.txt', delimiter='\t', unpack=True)
A3,dA3,B3,dB3,C3,Temp3=datafile3[a],datafile3[da],datafile3[b],datafile3[db],datafile3[c],datafile3[4]
datafile4 = np.loadtxt('Messung4_Black_body_0.txt', delimiter='\t', unpack=True)
A4,dA4,B4,dB4,C4,Temp4=datafile4[a],datafile4[da],datafile4[b],datafile4[db],datafile4[c],datafile4[4]
datafile5 = np.loadtxt('Messung5_Black_body_0.txt', delimiter='\t', unpack=True)
A5,dA5,B5,dB5,C5,Temp5=datafile5[a],datafile5[da],datafile5[b],datafile5[db],datafile5[c],datafile5[4]
datafile6 = np.loadtxt('Messung6_Black_body_0.txt', delimiter='\t', unpack=True)
A6,dA6,B6,dB6,C6,Temp6=datafile6[a],datafile6[da],datafile6[b],datafile6[db],datafile6[c],datafile6[4]
datafile7 = np.loadtxt('Vollständige_Temp_Kurve_0_Temp_ohne_Leistung.txt', delimiter='\t', unpack=True)
A7,dA7,B7,dB7,C7,TempV=datafile7[a],datafile7[da],datafile7[b],datafile7[db],datafile7[c],datafile7[4]
...
D = np.array([(A7+B7)/2,(A+B)/2,(A1+B1)/2,(A2+B2)/2,(A3+B3)/2,(A4+B4)/2,(A5+B5)/2,(A6+B6)/2])
dD = [(dA7+dB7)/2+0.5,(dA+dB)/2+0.5,(dA1+dB1)/2+0.5,(dA2+dB2)/2+0.5,(dA3+dB3)/2+0.5,(dA4+dB4)/2+0.5,(dA5+dB5)/2+0.5,(dA6+dB6)/2+0.5]
E = [C7,C,C1,C2,C3,C4,C5,C6]
dE = [0.01,0.044,0.02,0.025,0.0313,0.077,0.0598,0.15]
dE = [C7*0.01+0.01,C*0.01+0.01,C1*0.01+0.01,C2*0.01+0.01,C3*0.01+0.01,C4*0.01+0.01,C5*0.01+0.01,C6*0.01+0.01]
datei1 = open('Leistung_und_Temp_für_0.txt','a')
datei1.write(str(D[0])+'\t'+str(dD[0])+'\t'+str(E[0])+'\t'+str(dE[0])+'\t'+str(TempV)+
...
popt,pcov,chi = fit_basic(func,D+273.15,E,p0=[0.0115,0],y_err=dE)
...
plt.errorbar(D+273.15,E,xerr=dD,yerr=dE, color='royalblue',fmt='.',label='m = (%s +/- %s)m² \nn = (%s +/- %s)m² \nchi²= %s'%(str(popt[0].round(5)),str(pcov[0].round(5)),str((popt[1].round(1)/(273.15+TempV)**4).round(5)),str((pcov[1].round(1)/(273.15+TempV)**4).round(5)),str(chi.round(2))))
...
plt.tight_layout()
plt.savefig("Fläche_aus_Leistung_0")
